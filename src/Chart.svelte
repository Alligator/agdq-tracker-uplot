...
export let series: [timestamps: number[], viewersSeries: number[], donationsSeries: number[]];
...
function renderChart(series, games) {
  ...
  chartEl.innerHTML = '';
  const valueFormatter = (self, rawValue: number) => rawValue ? '$' + uPlot.fmtNum(Math.round(rawValue)) : rawValue;
  const opts: Options = {
    id: 'chart1',
    ...
    axes: [
      ...
      },
      {
        ...
        scale: 'donations',
        stroke: '#705060',
        values: (self, ticks) => ticks.map((tick) => valueFormatter(self, tick)),
      },
    ],
  };
